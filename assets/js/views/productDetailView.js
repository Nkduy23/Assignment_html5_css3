...
if (CATEGORY_CONFIG[category]) {
  ...
  fetchData(jsonFile)
    .then((data) => {
      // Xác định key sản phẩm dựa trên category
      ...
      if (data[productKey] && Array.isArray(data[productKey])) {
        ...
        displayError(`Không tìm thấy sản phẩm cho danh mục "${category}".`);
      }
    })
    .catch((error) => console.error("Lỗi khi lấy dữ liệu:", error));
} else {
  displayError("Loại sản phẩm không hợp lệ!");
}
...
function updatePrice(price) {
  ...
  if (priceElement) {
    // Format giá trị với đơn vị tiền tệ
    const formattedPrice = new Intl.NumberFormat('vi-VN', {
      style: 'currency',
      currency: 'VND',  // Chỉnh sửa theo đơn vị tiền tệ bạn muốn (ví dụ: 'USD', 'VND', v.v.)
    }).format(price);
...
function updateThumbnails(thumbnails) {
  ...
  thumbnailsContainer.innerHTML = thumbnails && Array.isArray(thumbnails)
    ? thumbnails
        .map(
          (thumb) =>
            `<img src="${thumb}" alt="Thumbnail" class="product-detail__thumbnail" />`
        )
        .join("")
